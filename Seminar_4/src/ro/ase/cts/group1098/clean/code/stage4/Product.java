package ro.ase.cts.group1098.clean.code.stage4;

// stage 4
// -> use the received productType and not the generic enum symbol
// -> remove the switch because all branches do the same stuff xd (except NEW)
...
public class Product {
	...
	public float computeFinalPrice(ProductType productType, float initialPrice, int accountAgeInYears) {
		...
		if (productType != ProductType.NEW) {
			accountDiscount = (accountAgeInYears > MAX_ACCOUNT_AGE) ? MAX_ACCOUNT_DISCOUNT
